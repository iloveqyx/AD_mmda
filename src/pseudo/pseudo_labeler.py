...
def make_pseudo_labels(
    ...
    sim_a:torch.Tensor,    # audio模态与原型的cos相似度 [B, C]
    sim_t:torch.Tensor,    # text模态与原型的cos相似度 [B, C]
    sim_f:Optional[torch.Tensor]= None,    # 融合模态与原型的cos相似度 [B, C]
    sim_a_logits:Optional[torch.Tensor]= None,  # audio模态与原型的logits [B, C]
    sim_t_logits:Optional[torch.Tensor]= None,  # text模态与原型的logits [B, C]
    sim_f_logits:Optional[torch.Tensor]= None,  # 融合模态与原型的logits [B, C]
    #===== 相似度阈值 =====
    theta_a: float = 0.8, # θ_a：音频原型cos相似度的置信度阈值
    theta_t: float = 0.8, # θ_t：文本原型cos相似度的置信度阈值
    ...
    kl_eps: Optional[float] = None  # KL 一致性阈值（可选）
    ...
    if sim_f is not None:
        ...
        p_proto = (
            F.softmax(sim_a_logits, dim=-1)
            + F.softmax(sim_t_logits, dim=-1)
            + F.softmax(sim_f_logits, dim=-1)
        ) / 3.0
    else:
        p_proto = (
            F.softmax(sim_a_logits, dim=-1)
    ...
    # 权重：用三路置信度的最小值，保守一点
    pa_w = F.softmax(sim_a_logits, dim=-1).max(dim=-1).values
    pt_w = F.softmax(sim_t_logits, dim=-1).max(dim=-1).values
    pseudo_w = torch.minimum(torch.minimum(pa_w, pt_w), ph)    # [B]
